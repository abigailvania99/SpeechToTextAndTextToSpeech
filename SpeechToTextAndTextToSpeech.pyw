...
##    os.system("start {}.mp3".format(fileName))
    ...
    buttons1 =Button(frame1, text="Get to Speech", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: entryRecognize(entry1, entry2))
    #canvas1.destroy
    #newWindow.destroy
    buttons1.pack()
...
def saveRecognize(frame, speechtext):
    ...
    buttons_yes =Button(frame, text="Save", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: [tulisan1.pack_forget(), entry1.pack_forget(), buttons_yes.pack_forget(), buttons_no.pack_forget(), saveToText(frame, speechtext, str(entry1.get()))])
    buttons_yes.pack()
...
def sttWindow():
    ...
    if i == 0:
        ...
        buttons_confirm=Button(frame1, text="Save", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: entryRecognize(entry1, entry2))
        buttons_no2=Button(frame1, text="Nevermind, go back to main menu", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: entryRecognize(entry1, entry2))


        buttons_yes =Button(frame1, text="Yes, save to txt", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: [saveRecognize(frame1, text), tulisan_text.pack_forget(), tulisan_asksave.pack_forget(), buttons_yes.pack_forget(), buttons_trys.pack_forget(), buttons_no.pack_forget()])
        buttons_yes.pack()

        buttons_trys =Button(frame1, text="Speak again", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: sttWindow())
        buttons_trys.pack()

        buttons_no =Button(frame1, text="No, go back to main menu", padx=10, pady=5, fg="white", bg="#263D42", command=mainMenu)
        buttons_no.pack()
...
##            print("Data has been saved")
    if i == 1:
        buttons_try =Button(frame1, text="Try again", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: sttWindow())
        buttons_try.pack()
        buttons_back =Button(frame1, text="Return to main menu", padx=10, pady=5, fg="white", bg="#263D42", command= lambda: mainMenu())
        buttons_back.pack()
